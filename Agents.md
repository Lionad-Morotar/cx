...
Schema 驱动的 Vue 组件渲染系统：从低代码组件定义（schema）到运行时渲染的完整链路。共 9 个物料/集成子包，形成 definition → vue → renderer → comps / comps-nuxt-ui-v2 / comps-nuxt-ui-v4 / comps-vtu / comps-tanstack-charts 等 → nuxt 的依赖链，外加 `playground` 开发沙箱。
