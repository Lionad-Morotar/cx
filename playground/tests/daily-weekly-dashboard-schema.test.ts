...
describe('日会 schema 静态骨架', () => {
  ...
  it('根为 daily page-layout，5 个具名 slot 各填正确内容物料', async () => {
    ...
    expect(asCmpt(slots['page-header']?.[0]).key).toBe('cx-daily-standup-header-info')
    expect(asCmpt(slots['page-header-right']?.[0]).key).toBe('cx-daily-page-actions')
    expect(asCmpt(slots['page-content-left']?.[0]).key).toBe('cx-daily-standup-filter')
    expect(asCmpt(slots['page-content-main']?.[0]).key).toBe('cx-daily-main-content')
    expect(asCmpt(slots['page-content-right']?.[0]).key).toBe('cx-user-select')
  })
})
...
describe('周会 schema 静态骨架', () => {
  ...
  it('根为 weekly page-layout，5 个具名 slot（名与日会不同）各填正确内容物料', async () => {
    ...
    const slots = root.components!
    expect(asCmpt(slots['page-header-center']?.[0]).key).toBe('cx-weekly-user-info-and-time')
    expect(asCmpt(slots['page-header-right']?.[0]).key).toBe('cx-weekly-page-actions')
    expect(asCmpt(slots['page-main-section']?.[0]).key).toBe('cx-weekly-main-content')
    expect(asCmpt(slots['page-aside-section']?.[0]).key).toBe('cx-weekly-todo-card')
    expect(asCmpt(slots['page-right-section']?.[0]).key).toBe('cx-user-select')
  })
})
