 ...
 * /dev/stream/components 与 /dev/stream/pages 两验收页共用——
 * 页面各自持有剧本源（Ref<string[]>），引擎只管推进与暂停，不感知剧本内容。
 */
export function useStreamReplay(chunks: Ref<string[]>) {
  ...
  const streamText = computed(() => chunks.value.slice(0, progress.value).join(''))
  ...
  return { playing, speed, progress, streamText, togglePlay, reset }
}
