...
description: |
  ...
  覆盖：调研目标库 → 建包骨架 → 逐组件包装为 define 物料 → 接入 cx-nuxt + playground 验收页 → 全量验证。
  也覆盖物料包建成后的两条成熟度链路：variants 深化展示（验收页全视觉属性对照）与
  Stream 流式改造（@lionad/cx-stream 增量渲染：数组增长型 / 标量主体形态声明、wrapper 骨架、回放链路）。
  另覆盖第二种转换场景：既有业务页 → 宿主本地物料（components/cx）的页面级迁移，
  含双页对比验收与路由不变式一键替换（references/page-migration.md）。
  ...
  - 用户要把既有业务页整体 cx 化为宿主本地物料，双页并行验证后一键替换旧页

  触发关键词：cx 物料、material bundle、define 物料、CxMaterialBundle、comps-vtu、comps-nuxt-ui、
  组件库转物料、包装组件、variants 深化、stream trigger、标量主体形态、流式回放、
  页面物料化、宿主本地物料、双页对比验收、一键替换、convert-to-cx-materials
...
- **一个物料** = `define({ key, name, description, icon, component, props, emits?, slots? })` 的返回值。
...
> 业务页 → 宿主本地物料的页面级迁移不走本流程（注册/消费/验收/收尾全不同），
> 见 [`references/page-migration.md`](references/page-migration.md)。
...
3. **S1 地基（端到端单组件，含样式实证）**：挑一个简单且标志性的组件，做完包装 SFC + define + bundle + cx-nuxt 注册 +
   playground 接线 + 验收页骨架，并在浏览器看到它带样式渲染。样式集成是首要风险，必须在此实证，不通过不铺量。
4. **批量铺量（按目标库官方分类切 Slice）**：每个组件 = 包装 SFC（复用同一 composable）+ define（props 映射 + 合规 initial 样本）。
...
3. **`define` 的编译期 Guard 对「包装组件空 `defineProps<{}>()` + 丰富 meta.props」恒真**——因为 SFC 类型不匹配构造签名使
...
9. **宿主页面物料若 import service 模块，meta 组件必须 `defineAsyncComponent` 包裹**——物料注册插件静态链
   求值早于 Nuxt 就绪，静态 import 连带执行 service 顶层 Nuxt composable，整页白屏；meta 的 `async?: boolean`
   只是标记位，不延迟模块求值，不能替代动态 import。（page-migration §不变量）
...
| 包骨架/包装 SFC/define/composable/cx-nuxt/验收页/分类/测试的可复制模板 + props 映射配方 + 构建/验证命令                                         | [`references/conversion-playbook.md`](references/conversion-playbook.md) |
...
| 业务页 → 宿主本地物料的页面级迁移：场景判定六轴、注册插件形态、五条不变量（异步包裹/前缀类透传/EP 非分层/供给驱动/定高捕获）、双页对比验收、路由不变式替换与死代码清除 | [`references/page-migration.md`](references/page-migration.md) |
