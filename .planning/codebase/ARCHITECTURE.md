...
│   `packages/renderer/src/comps/render.vue`              （CxRender）  │
│   `packages/renderer/src/comps/render-component.vue`    （递归）      │
│   `packages/renderer/src/comps/render-components.vue`   （插槽集合）  │
└──────────────────────────────┬──────────────────────────────────────┘
...
| `CxRender`                                 | 顶层渲染组件；提供上下文（`cx` / `is-cx-edit` / `cx-render-parent` 等）并挂 `Suspense`                             | `packages/renderer/src/comps/render.vue`                                                                                          |
...
- **流式增量渲染（cx-stream）**：从 LLM 流式输出的不完整 JSON 提取可渲染前缀树。物料以 `StreamTriggerConfig` 声明三种形态——array（主数组逐行）、region（slot 分区揭示）、scalar（骨架字段）；`compileTrigger` 编译组件级 trigger（scanPaths 计数，服务单围栏单物料），`compileTreeTrigger` 编译页面级树级 trigger（闭合事件分支：管线截断保证 parsed 值皆完整，buildPartial 深递归按节点 key 应用各物料声明，无声明节点退化为修剪，管线 core 零改动）。验收页：`/dev/stream/components`（组件级）、`/dev/stream/pages`（页面级嵌套树）。
...
- **包含：** `comps/`（5 个渲染器组件 + `info.vue`）、`event/native-event.ts`（原生事件清单）、`styles/index.scss`（`@layer cx`）、`utils/`（`deepMerge`）、`shims.d.ts`（样式副作用声明）。
- **依赖：** `@lionad/cx-definition`、`@lionad/cx-vue`、`sass-embedded`。
- **被使用：** comps、comps-nuxt-ui-v4、nuxt；并通过 `CxRenderComps`（`packages/renderer/src/comps/index.ts`）作为可被安装的物料集合。

### comps 层（`packages/comps/src/`）

- **职责：** 自研基础物料。
- **位置：** `packages/comps/src/`
...
### comps-nuxt-ui-v4 层（`packages/comps-nuxt-ui-v4/src/`）

- **职责：** 基于 vendored Nuxt UI v2 的物料库（40+ 组件：button、input、card、modal、table、form 等）。
- **位置：** `packages/comps-nuxt-ui-v4/src/`
- **包含：**
...
5. `CxRender`（`packages/renderer/src/comps/render.vue`）挂 `Suspense`，`provide('cx', cx)`，把首个组件交给 `CxRenderComponent`。
6. `CxRenderComponent`（`packages/renderer/src/comps/render-component.vue`）按 `cmpt.key` 经 `resolveComponent` 拿到 Vue 组件，合并 `cmpt.data` → props、`cmpt.emits` → 事件、`cmpt.slots` → 插槽，再包一层 `CxRenderComponentWithBindings`（指令 + cx-styles）。
...
**状态管理：** 无 Pinia/Vuex。CxLoader 自身维护 `installed`（注册表）、`refs`（`RefsManager` 按 id 存实例）、`datas`（响应式状态，含 `comps` / `root`）、`emitter`。playground 站会模块有自有的 `states/` 目录（pinia-like 风格的 plain composable stores：`standup.ts`、`standups.ts`、`project.ts`、`users.ts`、`issues.ts`、`issue-filter.ts`、`project-last-selected.ts`）。
...
- **使用例：** `packages/comps/src/grid/index.ts`、`packages/comps-nuxt-ui-v4/src/nuxt-ui-2/button/index.ts`、`packages/comps-nuxt-ui-v4/src/simple-card/index.ts`。
...
- **挂载属性：** `hooks`（loader 钩子，`readonly`）、`utils`（`createCxUtils` 工厂产出，`readonly`）、`refs`（`RefsManager`）、`emitter`（mitt 风格）、`datas`（`createCxDatas`，包含 `root` / `comps` / `renderCmptList`）、`installed` / `installedAsync` / `installedComponents`、`metadata`。

### `CxRenderComps`（渲染器物料集）

- **用途：** 把 `CxRender`、`CxRenderComponent`、`CxInfo` 等本身作为物料暴露给编辑器，方便 p-ray 在画布里嵌套使用。
- **位置：** `packages/renderer/src/comps/index.ts`
...
- `packages/renderer/src/index.ts`：导出 `CxRender`、`CxRenderComps`，引入 `styles/index.scss`。
- `packages/comps/src/index.ts`：导出 `CxBasics` / `CxGrid` / `CxCalendar` / `CxPage` / `CxUserStyle`。
- `packages/comps-nuxt-ui-v4/src/index.ts`：导出 `CxNuxtUI`（40+ 物料数组）+ `CxSimpleCard`。
...
- **Nuxt 虚拟模块离线化：** `vite.config.ts` 与 `packages/comps-nuxt-ui-v4/tsconfig.json`、`packages/comps-nuxt-ui-v4/vite.config.ts` 共同把 `#app` / `#imports` / `#build/app.config` / `#ui-colors` / `nuxt/schema` alias 到 `packages/comps-nuxt-ui-v4/vendor/shims/*`。**禁止把 shim 路径改成"真实 Nuxt"** —— 这是 vendored 物料脱离 Nuxt 运行时打包的必要条件。
...
- **Suspense 不能移除：** `packages/renderer/src/comps/render.vue` 模板顶层是 `<Suspense>`，依赖断点宽度（来自 `$cxRenderParent`），去掉会让 parent 变成 Fragment 导致断点计算失败。
...
**正确做法：** 一律走 `normalize({ name, key, icon, description, component, props, ... })`（参考 `packages/comps/src/basic/index.ts`）。
...
**正确做法：** 真实 Nuxt 宿主由 Nuxt 自身解析虚拟模块；只在 `@lionad/cx-comps-nuxt-ui-v4` 独立打包测试或非 Nuxt 消费时使用 shim。
...
**正确做法：** 保留 `Suspense` + `renderKey`（见 `packages/renderer/src/comps/render.vue`）。
...
- 单组件渲染失败：`CxRenderComponent` 用 `onErrorCaptured` + `showCmptErrorWrapper` 切换为错误态包装组件（`packages/renderer/src/comps/render-component.vue`）。
...
- `CxRender` watchEffect 监听 `cxTranslateFn.value` 变化触发重渲染（`packages/renderer/src/comps/render.vue`）。

**断点（响应式）：**

- `useCxBreakpointType`（`packages/vue/src/hooks/use-cx-props/use-cx-styles/`）以 `cxRenderParent` 宽度为来源；`CxRenderComponentWithBindings` 缓存按 `cxRenderRef` 复用 hook 实例（`packages/renderer/src/comps/render-component-with-bindings.vue`）。
...
- 物料样式位于 `@layer cx` 与 `packages/comps-nuxt-ui-v4/src/styles/`。

**日志：** 全仓使用裸 `console.log` / `console.info` / `console.error` / `console.group`，无统一 logger；调试日志以 `[info]` / `[debug]` / `[ERR]` 前缀区分。

**校验：** `zod` 用于部分表单 schema（`packages/comps/src/basic/index.ts`、`packages/comps-nuxt-ui-v4`）。
