 ...
 * `cmpt`、class/style 与 data-* 编辑标记一并作为 attrs 灌入物料包装层。透传给 vtu 前需剥离
 * 不属于 vtu props 的 cx 内部键：`cmpt`（运行时节点，落到 DOM 会变成 "[object Object]" 属性）、
 ...
 * 故缺省时回退到 cx 运行时节点的唯一 cmpt.id，保证 id 恒有值且同页多实例互不冲突。
 */
export function useVtuProps<T extends object>(
  ...
  return computed(() => {
    ...
    for (const [key, value] of Object.entries(attrs)) {
      if (key === 'cmpt' || key.startsWith('data-') || key.startsWith('_')) {
        continue
      }
      rest[key] = value
    }
