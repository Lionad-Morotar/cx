<template>
  <span style="display: none" aria-hidden="true" class="cx-computed" />
</template>
...
/**
 * 派生状态物料：用受限表达式（safeEval）对 ctx 求值，结果写入 comp.data.value，
...
watchEffect(() => {
  try {
    props.comp.data.value = safeEval(props.expr, props.ctx ?? {})
  } catch (e) {
    console.warn('[cx-computed] 表达式求值失败:', props.expr, e)
    props.comp.data.value = undefined
  }
})
</script>
