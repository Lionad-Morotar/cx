 ...
 * 写进自身 cmpt.data，供其他物料经 _cx_data_config 按同名 key 绑定；成功/失败
...
async function exec(...runtimeArgs: any[]) {
  props.cmpt.data.loading = true
  props.cmpt.data.error = null
  try {
    const data = await props.action?.(...(props.args ?? []), ...runtimeArgs)
    props.cmpt.data.data = data
    emit('success', data)
  } catch (err) {
    props.cmpt.data.error = err
    emit('error', err)
  } finally {
    props.cmpt.data.loading = false
  }
}
