 ...
 * 随完整 JSON 终态帧兜底；空主数组无可切分边界，全程无帧、终态 null（未开 emptyPassthrough）。
 */
const ARRAY_KEYS = [
...
describe('array 尾随标量与空主数组', () => {
  it('collapse：尾随标量 accordion 不进增量帧、随终态帧兜底', () => {
    // accordion 序列化在 items 之后：增量帧只认主数组，尾随标量交完整帧；
    // 完整 JSON 输入经主路径直出终帧（EP 同构断言范式），尾随标量随终帧兜底可查
    ...
    const registry = createNaiveUiTriggerRegistry()
    const extractor = createIncrementalExtractor<CxSpec>({ registry, matchTrigger: matchCxTrigger })
    const incremental: CxSpec[] = []
    for (let i = 1; i < text.length; i++) {
      const frame = extractor.next(text.slice(0, i))
      if (frame) incremental.push(frame)
    }
    expect(incremental.length).toBeGreaterThan(0)
    for (const frame of incremental) {
      const data = (frame as { data?: Record<string, unknown> }).data
      expect(data?.accordion, '尾随标量不应进增量帧').toBeUndefined()
    }
    const final = extractor.next(text) as { data?: Record<string, unknown> } | null
    expect(final, '终态兜底应直出完整帧').not.toBeNull()
    expect(final?.data?.accordion, '尾随标量应随终态帧兜底').toBe(true)
  })
