...
export interface CxChartPropsPartition {
...
/**
 * 把 cx 渲染器灌入的 attrs 分馏为 { spec, hostProps, ariaLabel } 三个独立 computed。
 *
 * 剥离 cx 内部键（comp 运行时节点、data-* 编辑标记、_ 前缀编辑器键），与 vtu/EP 桥接同形；
 * 额外把 definition 键从透传集中分馏——Chart 组件的 definition prop 只接受运行时产物，
