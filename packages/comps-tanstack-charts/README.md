...
TanStack Charts（`@tanstack/vue-charts`）的 cx 物料包：以纯 JSON schema 驱动 `defineChart` 的声明式翻译层 + 物料集。

- 通用物料 `cx-tanstack-charts-chart`：完整 JSON definition 投影（marks/x/y/theme/tooltip 标量子集）
