...
describe('createCxCmptRuntime', () => {
  ...
  it('children 透传到 components（slot → 节点数组）', () => {
    const child = createCxCmptRuntime('child', 'cx-text')
    const parent = createCxCmptRuntime('parent', 'cx-block', { default: [child] })

    expect(parent.components).toEqual({ default: [child] })
  })

  it('data 透传并保留原始键值', () => {
    const c = createCxCmptRuntime('id', 'cx-card', {}, { theme: 'dark', count: 3 })

    expect(c.data).toEqual({ theme: 'dark', count: 3 })
  })

  it('产物满足 isCxComponent 守卫', () => {
    const c = createCxCmptRuntime('id', 'cx-card')

    expect(isCxComponent(c)).toBe(true)
  })

  it('cxNode 是 createCxCmptRuntime 的别名（同一引用）', () => {
    expect(cxNode).toBe(createCxCmptRuntime)
  })

  it('支持嵌套 children 构造多 slot 树', () => {
    const root = createCxCmptRuntime('root', 'cx-page', {
      header: [createCxCmptRuntime('header', 'cx-header')],
      main: [createCxCmptRuntime('main', 'cx-main')],
    })
  ...
  it('返回类型严格为 CxComponentRuntime（类型层断言，由 typecheck 裁决）', () => {
    const c: CxComponentRuntime = createCxCmptRuntime('id', 'cx-card')
