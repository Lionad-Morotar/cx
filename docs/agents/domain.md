...
cx 采用 **single-context（单上下文）** 布局：8 个子包（definition → vue → renderer → comps → comps-nuxt-ui-v2 → nuxt）是同一套 schema 驱动渲染系统的分层，共享统一领域语言，故只需一份根 `CONTEXT.md`，而非 multi-context 的每包一份。
